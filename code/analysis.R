...
for (i in 1:length(treatments)) {
	...
	dat[ivs[i]] = c(rep(0, n), rep(1, n))
	control_n <- sapply(eths, function (x) nrow(data[data[ivs[i]] == 0 & grepl(x, data$ethnicity),]))
	treat_n <- sapply(eths, function (x) nrow(data[data[ivs[i]] == 1 & grepl(x, data$ethnicity),]))
	dat$n <- c(control_n, treat_n)
	dat$prop <- c(control_n/nrow(data[data[ivs[i]] == 0, ]), treat_n/nrow(data[data[ivs[i]] == 1, ]))
